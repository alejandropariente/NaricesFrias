        ...
        public AnimalDateImpl()
        {
            this.querys = new KeyQuery[5] {
                ...
                new KeyQuery("Update", @"BEGIN TRANSACTION
                                        UPDATE Animal SET name = @name , animalBreed = @animalBreed , age = @age , animalCategoryId = @animalCategoryId , userId = @userId WHERE id = @id
                                        UPDATE ShelterAnimal SET photo = @photo , shelterId = @shelterId WHERE id = @id
                                        COMMIT") ,
                new KeyQuery("Get",@"SELECT A.id , A.name , A.animalBreed , A.age , A.animalCategoryId , SA.photo , SA.shelterId
                                    FROM Animal A
                                    INNER JOIN ShelterAnimal SA ON A.id = SA.id WHERE id = @id") ,
